# Event Processor Migration System

This is a standalone migration system for the event-processor that uses `golang-migrate` to manage database schema changes with a custom `processor_migrations` table.
...
```bash
# From the event-processor root directory
make migrate-up
make migrate-down
make migrate-status
```
...
The system uses golang-migrate with a custom `processor_migrations` table to track applied migrations, separate from other modules' migration tables.

## Features

- **golang-migrate**: Uses the industry-standard golang-migrate library
- **Custom Migration Table**: Uses `processor_migrations` table (separate from other modules)
...
- **Standalone**: Runs independently from the main processor
