...
pub fn assert_fossil_callback_success_event(
    ...
    let _ = pop_log::<Vault::Event>(vault_address); // Pop the first (round deployed event)
    let _ = pop_log::<Vault::Event>(vault_address); // Pop the second (callback success event)
