...
export class TestRunner {
    ...
    accelerateToAuctioning = async () => {
        const optionRoundContract = await getOptionRoundContract(this.provider, this.vaultFacade.vaultContract);
        ...
        await timeskipNextBlock(Number(auctionStartDate) - Number(currentTime), this.provider.channel.nodeUrl);
    };
    accelerateToRunning = async () => {
        const optionRoundContract = await getOptionRoundContract(this.provider, this.vaultFacade.vaultContract);
        const currentTime = await getNow(this.provider);
        const auctionEndDate = await optionRoundContract.get_auction_end_date();
        await timeskipNextBlock(Number(auctionEndDate) - Number(currentTime) + 1, this.provider.channel.nodeUrl);
    };
    accelerateToSettled = async () => {
        const optionRoundContract = await getOptionRoundContract(this.provider, this.vaultFacade.vaultContract);
        const currentTime = await getNow(this.provider);
        const optionSettleDate = await optionRoundContract.get_option_settlement_date();
        await timeskipNextBlock(Number(optionSettleDate) - Number(currentTime), this.provider.channel.nodeUrl);
    };
    ...
    //@note Only works for katana dev instance with a --dev flag
    settleOptionRoundBystander = async (marketData) => {
        console.log("MARKETDATA:", marketData);
        const factRegistryString = await this.vaultFacade.vaultContract.get_market_aggregator_address();
        const factRegistryAddress = "0x" + stringToHex(factRegistryString);
        const factRegFacade = new FactRegistryFacade(factRegistryAddress, this.provider);
        const optionRound = await getOptionRoundFacade(this.provider, this.vaultFacade.vaultContract);
        // Mock JobRequest
        const jobRequest = await optionRound.createJobRequest();
        const devAccount = getAccount("dev", this.provider);
        await factRegFacade.setMarketParameters({
            devAccount,
            jobRequest,
    ...
    getLiquidityProviderAccounts = (length) => {
        ...
        for (let i = 0; i < length; i++) {
            liquidityProviderAccounts.push(getCustomAccount(this.provider, liquidityProviders[i].account, liquidityProviders[i].privateKey));
        }
    ...
    getOptionBidderAccounts = (length) => {
        ...
        for (let i = 0; i < length; i++) {
            optionBidderAccounts.push(getCustomAccount(this.provider, optionBidders[i].account, optionBidders[i].privateKey));
        }
