...
export const smokeTest = async ({ provider, vaultFacade: vault, getBalancesAll, getOptionBidderAccounts }) => {
    const optionRoundFacade = await getOptionRoundFacade(provider, vault.vaultContract);
    const optionRoundERC20Contract = new ERC20Facade(optionRoundFacade.optionRoundContract.address, provider);
    ...
    const optionBalancesBefore = await optionRoundFacade.getTotalOptionsBalanceForAll(optionBidderAccounts);
    try {
    ...
    try {
        ...
        await optionRoundERC20Contract.erc20Contract.approve(optionRoundERC20Contract.erc20Contract.address, BigInt(totalOptionAvailable) / BigInt(4));
        ...
        // });
        await optionRoundERC20Contract.erc20Contract.transfer(optionBidderAccounts[1].address, BigInt(totalOptionAvailable) / BigInt(4));
    }
    catch (err) {
        console.log("Error while transferring the tokenized options", err);
    }
    const optionBalancesAfter = await optionRoundFacade.getTotalOptionsBalanceForAll(optionBidderAccounts);
    checkpoint2({
...
async function checkpoint1({ balancesBefore, balancesAfter, totalOptionAvailable, reservePrice, }) {
    assert(BigInt(balancesBefore[0]) +
        (BigInt(totalOptionAvailable) / BigInt(2)) * BigInt(reservePrice) ===
        BigInt(balancesAfter[0]), "Unused bids balance fail");
    assert(BigInt(balancesBefore[1]) +
        (BigInt(totalOptionAvailable) / BigInt(2)) * BigInt(reservePrice) ===
